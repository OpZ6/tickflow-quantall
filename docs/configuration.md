...
**设置 → 数据源** 按日 K、除权、实时、分钟 K、五档和财务分别选择主数据源。一次选择只会把该数据源提升为当前能力的第一优先级，不会重置其他能力。每项能力保留独立的主备链；请求失败后系统会把故障源短暂冷却 60 秒并尝试链中下一来源，恢复后的来源可在后续请求中重新参与路由。

能力矩阵会同时显示配置的主备链、当前实际生效来源和运行健康状态。成功发布的数据来源与最近故障会写入 `data/user_data/data_source_lineage.json`，便于核对一次结果究竟来自哪个数据源；实时行情和五档返回记录也带 `source` 字段。

`全量分钟` 是独立的全市场批量落盘能力，当前保持不可用。系统不会把逐股 TDX 请求拼成全量分钟源；只有接入支持全市场批量/流式交付并具备限流、断点续传和完整性校验的专用来源后才会开放该能力。

以上路由只作用于 TickFlow 研究/行情服务，不改变 QuantX 面板自己的采集器、事实库或数据源配置。
