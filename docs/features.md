...
盘后管道(默认 16:30 CST 自动触发,None/Free 档当日日 K 盘后约 1-2 小时可用)会重新拉日 K + 重算 enriched 表。
...
以一张统一 ECharts 为主体的单标的研究页:

- **统一周期与口径**:1/5/15/30/60 分钟、日/周/月,不复权/前复权/后复权,预设或自定义范围,并显示数据覆盖与预热质量
- **完整技术指标**:20 个主图指标、38 个副图指标和成交量可多选;支持搜索、参数、排序、折叠、高度、自定义预设和持久化
- **11 类关键价位**:由当前周期、复权和范围的同一组 candles 计算,可逐组显隐
- **结构研究**:缠论包含处理/分型/笔/段/中枢/买卖点、本地/官方严格对齐对照、五类形态、事件标记和用户画线
- **专业工具**:统一十字光标/缩放、截图、全屏、逐根无未来回放、单股分钟历史补齐
- **AI 四维分析**:技术 / 基本面 / 财务 / 消息面流式生成,客观技术分析视角(不输出买卖建议)

详细契约见 [stock-chart-workbench.md](./stock-chart-workbench.md)。
...
APScheduler 默认 16:30 CST 自动(可配置,下限 15:00):拉日 K → 重算 enriched 表 → 跑监控规则。
