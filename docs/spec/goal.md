...
>
> 2026-08-31 后续收口：VCP、杯柄、高而紧旗形、启动后缩量回踩由观察形态提升为正式注册策略；其旧 `pattern.*` 入口已移除。所有买卖标记都必须来自正式注册策略：策略面板/回测/监控的持久化证据进入 `strategy_signal_events`，而显式声明 `chart_preview` 的策略允许对当前单股按需、只读地回放历史信号。即时预览不执行全市场扫描、不写入事件仓库，也不模拟成交；下文把四类能力描述为独立形态的内容仅保留为原始设计背景，不再覆盖这一现行约束。
>
