...
- TickFlow 全景架构报告（本地可选产物 `docs/tickflow-architecture-report.html`）：面向产品、数据和二次开发者的可视化 HTML 导读，解释数据层、处理层、展示层、两类数据入口、存储、QuantX 独立流水线、插件与新增功能路径。该文件不属于源码仓库；具体契约仍以下方六份权威文档和当前源码为准。
