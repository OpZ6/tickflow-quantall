...
同日第二轮收口：顶部交易日切换改用日历日期选择器，并只允许选择已发布记录；交易日情绪分数提升背景、边框和文字对比度。参与度条件并入情绪风险雷达，顶部重复关注池删除，只保留深度区完整关注名单。跨日生灭按新生、增强、延续、转弱、退出分组并去除重复事件；行业资金连续性与连续规则候选固定左右双列展示。

同日第三轮信息架构收口：行业资金连续性只保留 `sector_flow_continuity` 一套展示，移除与其数据完全相同的机构连续性副本；“今日决断”不再重复风险列表，并与仓位、三种次日场景和复盘摘要合并为关注阶段的“决断区”。5/10/20 日窗口信号矩阵与主线、升温、降温题材合并为同一张多日题材工作台，窗口统计紧随其后。资金分布、行业宽度、行业流入流出、涨跌幅与净流入、行业及规则连续性、核心个股统一归入“资金与行业验证”工作区。深度流程固定为市场趋势、情绪连板、题材行业、资金与行业验证、关注与决断、完整数据、质量血缘，形成从市场判断到行动决断的单向阅读顺序。
