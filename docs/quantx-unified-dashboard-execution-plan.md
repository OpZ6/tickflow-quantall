...
5. 重型图表只渲染可见区域；“全部展开”仍在同一页面按需挂载全部图表。
...
| `view` | `compact/full` | 本地偏好或 `compact` | 紧凑与全部展开 |
...
目标：让所有原图表和表格在同页可访问，并提供全部展开模式。

改动：

- 实现市场、题材行业、情绪连板、资金、关注、完整数据、质量七个页签；
- 默认只挂载活动页签的重型ECharts；
- `view=full` 按领域逐段挂载全部图表；
- 图表高度在紧凑态压缩，完整态使用原语义所需高度；
...
必须检查：日期、窗口、行业层级、题材选择、个股弹窗、深度页签、紧凑/全部展开、刷新、空数据、部分发布、来源降级和错误状态。
